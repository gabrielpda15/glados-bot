...
import { ActionRowBuilder, ButtonBuilder, ButtonComponentData, ButtonStyle, CollectedInteraction, ComponentType, EmbedBuilder, InteractionCollector, Message, PermissionsString } from 'discord.js';
...
export class Queue implements DiscordCommand {
    ...
    public async execute(e: DiscordCommand.MessageExecuteArgs | DiscordCommand.InteractionExecuteArgs): Promise<any> {
        ...
        if (e.isMessage())
            embedMessage = await e.send({ embeds: [ embed.data ], components: components });
